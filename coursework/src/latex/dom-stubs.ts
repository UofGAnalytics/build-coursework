// This file currently should be included when running pdf.js in Node.js environment
    ...
    switch (this._state) {
      ...
      case 3: // Serialize any attributes and end opening tag.
        if (this._loopIndex < this._attributeKeys.length) {
          ...
          return (
            ' ' + name + '="' + xmlEncode(node.attributes[name]) + '"'
          );
        }
        ++this._state;
        return '>';
...
// export function setStubs(namespace) {
