...
p <- autoplot(model.ma1$residuals, main="MA(1) residual series") + 
              geom_line(colour="#4a1486")
pacf <- autoplot(acf(model.ma1$residuals, plot = FALSE), 
                ylab="ACF", main="")
ppacf <- autoplot(pacf(model.ma1$residuals, plot = FALSE),  
                   ylab="PACF", main="")
grid.arrange(p, pacf, ppacf, nrow=3)
```
...
###[example]
Below you see realisations of an ARIMA(1,1,0) (left column) and ARIMA(0,1,1) (right column) processes. Note the data are non-stationary and have a trend. Therefore the ACF is not informative regarding the presence or absence of short-term correlation.
...
The graphs below show the original data which is clearly not stationary. Taking the first order difference seems to make the series stationary with zero mean. Therefore $d=1$ appears to be adequate.
...
grid.arrange(diffacf, diffpacf, nrow=2)
```
A large ACF at lag 1 and a PACF that tails off to zero suggest an MA(1) model for this series, which would suggest an ARIMA(0,1,1) for the original `Nile` data.
...
```{r, fig.width=6, fig.height=4, fig.align='center'}
resacf <- autoplot( acf(fitNile$residuals, plot = FALSE), main="ACF for residuals of ARIMA(0,1,1)")
respacf <- autoplot( pacf(fitNile$residuals, plot = FALSE), main="PACF for residuals of ARIMA(0,1,1)")
...
Our final topic in this brief introduction to time series analysis is forecasting.
...
plot(1:465,c(model$fitted.values[2558:n], model.predict$fit[ ,1]), type="l", lwd=2,
...
plot(c(1:36), c(model$fitted.values, model.predict$fit[ ,1]), type="l", 
     ...
     xlab="Quarter", col="#fc4e2a", lwd=2)
points(x, pch=19, col=alpha("#b10026",0.7))
lines(33:36, model.predict$fit[ ,2], col="#fd8d3c", lwd=2, lty=2)
lines(33:36, model.predict$fit[ ,3], col="#fd8d3c", lwd=2, lty=2)
```
...
Exponential smoothing is a simple procedure that does not assume a parametric model for the data, and is similar to moving average smoothing discussed in [Week 6](http://moodle2.gla.ac.uk/pluginfile.php/1508882/mod_resource/content/3/week6.pdf). It makes one step ahead forecasts of the form
...
```{r echo=5:20, fig.width=6, fig.height=5, fig.align='center'}
library(forecast)
airtraffic <- read.csv(url("http://www.stats.gla.ac.uk/~tereza/rp/airtraffic.csv"))
x <- airtraffic[ ,3]
...
To allow for additive trend and seasonality we use:
...
plot(1:120, c(x, predict.ar1$pred), type="l", xlab="Time", ylab="Data", 
     lwd=2, ylim=c(-7,7), main="Forecast for an AR(1) series")
lines(101:120, predict.ar1$pred, col="#fc4e2a", lwd=2)
lines(101:120, predict.lci, lty=2, col="#fd8d3c", lwd=2)
lines(101:120, predict.uci, lty=2, col="#fd8d3c", lwd=2)
...
####[example]
Consider an AR(1) time series process $X_{t}=0.9X_{t-1}+Z_{t}$, where $\hat{\sigma}^{2}_{z}=1$ and $x_{n}=20$. Calculate the one- and two-step ahead forecasts and the associated error variances.
...
# Plotting
plot(1:120, c(x, predict.ar1$pred), type="l", xlab="Time", ylab="Data", 
     lwd=2, main="Forecast for an MA(3) series")
lines(101:120, predict.ar1$pred, col="#fc4e2a", lwd=2)
lines(101:120, predict.lci, lty=2, col="#fd8d3c", lwd=2)
lines(101:120, predict.uci, lty=2, col="#fd8d3c", lwd=2)
...
####[example]
Consider modelling the short time series $\mathbf{x}=(3,8,2,5,6)$  with an MA(1) time series process $X_{t}=0.7Z_{t-1}+Z_{t}$, where $\hat{\sigma}^{2}_{z}=1$. Calculate the one- and two-step ahead forecasts $x_{5}(1)$ and $x_{5}(2)$ as well as their associated error variances.
...
# Plotting
plot(1:120, c(x, predict.ar1$pred), type="l", xlab="Time", ylab="Data", 
     ylim=c(30,47), lwd=2, main="Forecast for AR(1) with linear trend")
lines(101:120, predict.ar1$pred, col="#fc4e2a", lwd=2)
lines(101:120, ar1.LCI, lty=2, col="#fd8d3c", lwd=2)
lines(101:120, ar1.UCI, lty=2, col="#fd8d3c", lwd=2)
```
...
###[task]
The file [`globaltemp.csv`](http://www.stats.gla.ac.uk/~tereza/rp/globaltemp.csv) gives the global temperature anomaly (annual-mean surface air temperature deviation from the 1951-1980 mean) obtained from meteorological station data. Use the `NoSmoothing` column to obtain a forecast for the next 30 years.
...
plot(temp$Year, temp$NoSmoothing, xlab="Year", ylab="Temperature Anomaly", type="l")

acf(temp$NoSmoothing)
```
...
**Chapter 3**: ARIMA Models (includes ARMA, ARIMA and discusses forecasting) 

Additional resources covering exponential smoothing methods for data with trend or seasonal variation can be found at this [link](http://uc-r.github.io/ts_exp_smoothing).
...
* forecast future values of a time series using regression methods
